...
class EventsTableViewCell: UITableViewCell {
    ...
    func configureCell(event:Event) {
        ...
        itemImageView.sd_setImage(with: URL(string: event.performers?.first?.image ?? ""), placeholderImage: UIImage(named: "placeholder.png"))
        cityLabel.text = event.venue?.displayLocation ?? ""
        dateTimeLabel.text = event.datetimeLocal?.formatedDateTime ?? ""
        if event.isFavouriteExistsInDB() {
